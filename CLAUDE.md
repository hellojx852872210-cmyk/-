...
## 数据存储架构决策（2026-05-10）
- 主存储：MySQL（全量样本、运行日志、用户鉴权、策略反馈）
- 展示层：飞书（汇总看板、人工协同、日报/告警推送）
- 原则：飞书不作为全量样本事实库，MySQL 才是唯一权威数据源。

### 落地约束
1. 新增数据链路默认先写 MySQL，再做飞书汇总推送。
2. 飞书表仅存聚合/摘要字段，避免承载高频明细写入。
3. 用户鉴权采用中心化（远程表/库）优先，本地配置仅作兜底。
4. 运行期产生的数据不再依赖本地文件作为长期留存。

### 演进顺序（执行建议）
1. 先接入 MySQL 写入：cycle 日志、改价落库、后验拦截、销售明细、用户 usage log。
2. 稳定后将飞书推送改为读取 MySQL 聚合结果。
3. 最后逐步下线本地 runtime 明细持久化，仅保留必要临时缓存/故障排查开关。
