...
def _settled_price_from_payload(payload: dict[str, Any]) -> Optional[float]:
    for key in ("settled_price", "settle_price"):
...
def _send_sales_status_report(notifier, wx_app_client, report: str) -> bool:
    ...
    if notifier is not None and hasattr(notifier, "send_text"):
        try:
            send_result = notifier.send_text(report)
            if isinstance(send_result, tuple):
                return bool(send_result[0])
...
def _build_status_notify_report(records: list[dict]) -> str:
    ...
    for index, record in enumerate(records, start=1):
        ...
        if (old_status == "未上架" and new_status == "在售") or new_status == "已售":
            model = str(record.get("model") or "-")
            condition = str(record.get("condition") or "-")
            capacity = str(record.get("capacity") or "-")
            settled_price = _settled_price_from_payload(record)
            settle_text = "-"
            try:
                if settled_price is not None:
                    settle_text = f"{float(settled_price):.0f}"
            except Exception:
                settle_text = "-"
            line += f" | 型号:{model} 成色:{condition} 内存:{capacity} 到手价:{settle_text}"
        if detail:
            line += f"（{detail}）"
        lines.append(line)
    return "\n".join(lines)
...
def _is_price_operation_blocked(account_name: str) -> bool:
    return str(account_name or "").strip().lower() == "ldy"
...
def _summarize_risk_bucket(risk_bucket: dict[str, int], decision: Optional[dict]) -> None:
    data = decision or {}
    risk_sources = list(data.get("risk_sources") or [])
    if not risk_sources:
        risk_sources = [str(data.get("risk_source_primary") or "none")]
    seen: set[str] = set()
    for source in risk_sources:
        ...
        if key in seen:
            continue
        seen.add(key)
        risk_bucket[key] = int(risk_bucket.get(key, 0) or 0) + 1
...
def _append_persisted_sample(
    ...
    *,
    ...
    if int(limit or 0) > 0 and len(samples) >= int(limit):
        return
    samples.append(
        {
            "task": task,
            ...
            "settle_price": round(float(settle_price or 0.0), 0) if settle_price is not None else None,
            "changed_at": (changed_at or datetime.datetime.now()).strftime("%Y-%m-%d %H:%M:%S"),
            "trigger": trigger,
        }
    )
...
def _is_small_price_diff(target_price: Optional[float], current_price: Optional[float]) -> bool:
    if target_price is None or current_price is None:
        return False
    return abs(float(target_price) - float(current_price)) < 1


def _pending_aware_op_status(skip_reason: str, default: str = "跳过") -> str:
    return "待确认" if "待人工确认" in str(skip_reason or "") else default


def _mark_small_diff_skip(
    imported_store: BatchItemStore,
    *,
    product_id: str,
    account_name: str,
    ...
    imported_store.update_item(
        ...
        op_status="跳过",
        op_message=f"{task_label}跳过｜{preview}",
    )
    _log(f"[{account_name}] [{item_mark}] 跳过: 差价小于 1 元｜{preview}", on_progress)


def _mark_manual_review_pending(
    imported_store: BatchItemStore,
    *,
    product_id: str,
    decision: Optional[dict],
    ...
    imported_store.update_item(product_id, **payload)
...
def task_erp_sync(
    ...
    try:
        ...
        fallback_by_erp_key: dict[tuple[str, str, str], BatchItem] = {}

        def _erp_key(erp_item) -> tuple[str, str, str]:
            return (
                str(getattr(erp_item, "product_id", "") or "").strip(),
                str(getattr(erp_item, "qc_code", "") or "").strip(),
                str(getattr(erp_item, "imei", "") or "").strip(),
            )
        ...
        for account in active_accounts:
            ...
            next_unresolved = list(unresolved)
            ...
            for erp_item, matched_item in matched_pairs:
                ...
                erp_key = _erp_key(erp_item)
                matched_status = ProductStatus.from_raw(
                    getattr(getattr(matched_item, "status", None), "value", getattr(matched_item, "status", ""))
                )
                if matched_status == ProductStatus.ON_SALE:
                    if matched_item.product_id not in added_product_ids:
                        matched_items.append(matched_item)
                        added_product_ids.add(matched_item.product_id)
                    resolved_keys.add(erp_key)
                else:
                    fallback_by_erp_key[erp_key] = matched_item
                    next_unresolved.append(erp_item)

            if resolved_keys:
                next_unresolved = [item for item in next_unresolved if _erp_key(item) not in resolved_keys]
            unmatched_items = next_unresolved
        ...
        if unmatched_items and fallback_by_erp_key:
            still_unmatched: list[object] = []
            for erp_item in unmatched_items:
                fallback_item = fallback_by_erp_key.get(_erp_key(erp_item))
                if fallback_item is None:
                    still_unmatched.append(erp_item)
                    continue
                if fallback_item.product_id not in added_product_ids:
                    matched_items.append(fallback_item)
                    added_product_ids.add(fallback_item.product_id)
            unmatched_items = still_unmatched
        ...
        if imported_store is not None:
            ...
            existing_by_imei: dict[str, list[BatchItem]] = {}
            for existing_item in existing_items:
                existing_imei = str(getattr(existing_item, "imei", "") or "").strip()
                if not existing_imei:
                    continue
                existing_by_imei.setdefault(existing_imei, []).append(existing_item)
            ...
            for index, matched_item in enumerate(matched_items, start=1):
                ...
                if pid in existing_ids:
                    ...
                    matched_status = ProductStatus.from_raw(getattr(getattr(matched_item, "status", None), "value", getattr(matched_item, "status", "")))
                    matched_imei = str(getattr(matched_item, "imei", "") or "").strip()
                    imei_candidates = existing_by_imei.get(matched_imei, []) if matched_imei else []
                    should_migrate = False
                    old_item = None
                    if matched_status == ProductStatus.ON_SALE and len(imei_candidates) == 1:
                        candidate = imei_candidates[0]
                        candidate_status = ProductStatus.from_raw(getattr(getattr(candidate, "status", None), "value", getattr(candidate, "status", "")))
                        candidate_review_state = str(getattr(candidate, "manual_review_state", "") or "").strip().lower()
                        candidate_op_status = str(getattr(candidate, "op_status", "") or "").strip()
                        if (
                            str(getattr(candidate, "product_id", "") or "").strip() != pid
                            and candidate_status == ProductStatus.OFF_SALE
                            and candidate_review_state != MANUAL_REVIEW_STATE_PENDING
                            and candidate_op_status != "待确认"
                        ):
                            should_migrate = True
                            old_item = candidate
                    if should_migrate and old_item is not None:
                        old_pid = str(getattr(old_item, "product_id", "") or "").strip()
                        if old_pid:
                            imported_store.remove(old_pid)
                            existing_ids.discard(old_pid)
                        matched_item.selected = bool(getattr(old_item, "selected", False))
                        matched_item.ignored = bool(getattr(old_item, "ignored", False))
                        matched_item.category = str(getattr(old_item, "category", "") or "")
                        old_imei = str(getattr(old_item, "imei", "") or "").strip()
                        if old_imei:
                            existing_by_imei.pop(old_imei, None)
                        migrated_count += 1
                    new_items.append(matched_item)
                    existing_ids.add(pid)
                    if matched_imei:
                        existing_by_imei.setdefault(matched_imei, []).append(matched_item)
                write_current = index
            ...
            _log(
                ...
                f"IMEI迁移 {migrated_count} 件，导入前 {existing_count_before} 件，导入后 {existing_count_after} 件",
                on_progress,
            )
            _emit_progress_event(
                on_progress_event,
...
def task_refresh_imported_items_status(
    ...
    *,
    limit: int = 80,
    on_status_change_record: Optional[Callable[[dict], None]] = None,
) -> dict:
    ...
    for item in candidates:
        ...
        if status_delta:
            ...
            new_status_label = _status_label(detail.status)
            status_detail = _status_detail_from_obj(detail)
            is_post_intercept = old_status_label == "已售" and new_status_label == "未上架"
            record = {
                ...
                "model": str(getattr(detail, "model", "") or getattr(item, "model", "") or ""),
                "condition": str(getattr(detail, "condition", "") or getattr(item, "condition", "") or ""),
                "settled_price": getattr(detail, "settle_price", None),
                "settle_price": getattr(detail, "settle_price", None),
                "old_status": old_status_label,
                "new_status": new_status_label,
                "status_detail": status_detail,
                "event_type": "status_regression" if is_post_intercept else "status_change",
                "is_post_intercept": is_post_intercept,
                "event_time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "source_channel": "status_refresh",
                "reason_hint": status_detail,
            }
            status_change_records.append(record)
            if on_status_change_record is not None:
                try:
                    on_status_change_record(dict(record))
                except Exception:
                    pass
        if price_delta:
            price_changed += 1
...
def task_auto_reprice(
    ...
    for item in items:
        ...
        if _is_price_operation_blocked(account.name):
            imported_store.update_item(
                item.product_id,
                reprice_ok=None,
                reprice_msg="账号策略限制：ldy 仅观察，不执行改价",
                op_status="跳过",
                op_message="自动调价跳过：账号策略限制",
            )
            _log(f"[{account.name}] [{item.qc_code or item.product_id}] 跳过: 账号策略限制（仅观察，不执行改价）", on_progress)
            skip += 1
            continue
        ...
        _summarize_risk_bucket(risk_bucket, decision)
        ...
        if _is_small_price_diff(new_price, detail.current_price):
            _mark_small_diff_skip(
                imported_store,
                product_id=item.product_id,
                account_name=account.name,
                item_mark=detail.qc_code or item.qc_code,
                preview=preview,
                task_label="自动调价",
                on_progress=on_progress,
            )
        ...
        if success:
            ...
            _append_persisted_sample(
                ...
                model=str(getattr(record_source, "model", detail.model) or ""),
                condition=str(getattr(record_source, "condition", detail.condition) or ""),
                qc_code=str(getattr(record_source, "qc_code", detail.qc_code) or item.qc_code or ""),
                imei=str(getattr(record_source, "imei", getattr(detail, "imei", "")) or ""),
                settle_price=settle,
...
def task_stale_drop(
    ...
    for item in items:
        ...
        if stale_days >= stage2:
            ...
            decision = recalc_decision_with_final_price(
                work_item,
                decision,
                stage_price,
                extra_steps=[f"阶段2：在架 {stale_days} 天，按滞销规则额外下调 {drop_pct}%"],
            )
            stage_label = "阶段2"
        else:
            ...
            decision = recalc_decision_with_final_price(
                work_item,
                decision,
                _normalize_tail8_price(base_price),
                extra_steps=[f"阶段1：在架 {stale_days} 天，按保守价策略复算"],
            )
            stage_label = "阶段1"
        ...
        if new_price is None or _is_small_price_diff(new_price, detail.current_price):
            _mark_small_diff_skip(
                imported_store,
                product_id=item.product_id,
                account_name=account.name,
...
def _probe_is_due(item: BatchItem, now: datetime.datetime, *, min_hours: int, daily_max: int) -> tuple[bool, str]:
    day_key = now.strftime("%Y-%m-%d")
    last_at = getattr(item, "probe_last_at", None)
    if isinstance(last_at, datetime.datetime):
        wait_seconds = max(int(min_hours or 0), 0) * 3600
        if wait_seconds > 0 and (now - last_at).total_seconds() < wait_seconds:
            return False, "未到探针周期"
    day_count = int(getattr(item, "probe_day_count", 0) or 0)
    saved_day = str(getattr(item, "probe_day", "") or "")
    if saved_day != day_key:
        day_count = 0
    if day_count >= max(int(daily_max or 0), 0):
        return False, "已达当日探针上限"
    return True, ""


def _probe_guard_window_hours(item: BatchItem) -> int:
    tier = _probe_tier(item)
    default_hours = 6 if tier == "fast" else 12
    configured = int(getattr(cfg, "probe_guard_window_hours", 0) or 0)
    if configured <= 0:
        return default_hours
    return configured if configured in {6, 12} else default_hours


def _probe_guard_triggered(item: BatchItem, *, now: datetime.datetime) -> tuple[bool, str]:
    product_id = str(getattr(item, "product_id", "") or "").strip()
    if not product_id:
        return False, ""
    db = get_history_db()
    window_hours = _probe_guard_window_hours(item)
    rows = db.query(days=max(int(window_hours / 24) + 2, 2), limit=80)
    cutoff = now - datetime.timedelta(hours=window_hours)
    recent = [r for r in rows if str(getattr(r, "product_id", "") or "") == product_id and getattr(r, "timestamp", now) >= cutoff]
    if not recent:
        return False, ""
    cum_drop_limit = max(float(getattr(cfg, "probe_guard_cum_drop_limit", 120) or 120), 0.0)
    big_drop_threshold = max(float(getattr(cfg, "probe_guard_big_drop_threshold", 50) or 50), 0.0)
    big_drop_count = max(int(getattr(cfg, "probe_guard_big_drop_count", 2) or 2), 1)
    cum_drop = sum(abs(float(getattr(r, "diff", 0.0) or 0.0)) for r in recent if float(getattr(r, "diff", 0.0) or 0.0) < 0)
    if cum_drop >= cum_drop_limit:
        return True, f"近{window_hours}小时累计降价过大（{cum_drop:.0f}）"
    drops = [r for r in recent if float(getattr(r, "diff", 0.0) or 0.0) <= -big_drop_threshold]
    if len(drops) >= big_drop_count:
        return True, f"近{window_hours}小时大幅降价次数过多（{len(drops)}次）"
    return False, ""


def _probe_tier(item: BatchItem) -> str:
    listed_time = getattr(item, "listed_time", None)
    if isinstance(listed_time, datetime.datetime):
        listed_days = max((datetime.datetime.now() - listed_time).days, 0)
    else:
        listed_days = 9999
    fast_days = max(int(getattr(cfg, "probe_fast_turnover_days", 7) or 7), 1)
    return "fast" if listed_days <= fast_days else "slow"


def _probe_params_for_item(item: BatchItem) -> dict[str, int | float | str]:
    tier = _probe_tier(item)
    if tier == "fast":
        return {
            "tier": tier,
            "delta": abs(float(getattr(cfg, "probe_fast_delta", 15) or 15)),
            "interval_hours": max(int(getattr(cfg, "probe_fast_interval_minutes", 180) or 180) // 60, 1),
            "daily_max": max(int(getattr(cfg, "probe_fast_daily_max", 8) or 8), 1),
        }
    return {
        "tier": tier,
        "delta": abs(float(getattr(cfg, "probe_slow_delta", 25) or 25)),
        "interval_hours": max(int(getattr(cfg, "probe_slow_interval_minutes", 360) or 360) // 60, 1),
        "daily_max": max(int(getattr(cfg, "probe_slow_daily_max", 4) or 4), 1),
    }


def _effective_probe_delta(item: BatchItem, base_delta: float) -> float:
    max_single_probe_delta = 20.0
    now = datetime.datetime.now()
    product_id = str(getattr(item, "product_id", "") or "").strip()
    if not product_id:
        return min(max(float(base_delta or 0.0), 1.0), max_single_probe_delta)
    window_hours = _probe_guard_window_hours(item)
    db = get_history_db()
    rows = db.query(days=max(int(window_hours / 24) + 2, 2), limit=120)
    cutoff = now - datetime.timedelta(hours=window_hours)
    recent_probe_count = 0
    for row in rows:
        if str(getattr(row, "product_id", "") or "") != product_id:
            continue
        if getattr(row, "timestamp", now) < cutoff:
            continue
        if getattr(row, "trigger", None) == PriceTrigger.PROBE:
            recent_probe_count += 1
    if recent_probe_count >= 3:
        return min(max(float(base_delta) * 0.7, 8.0), max_single_probe_delta)
    if recent_probe_count == 0:
        return min(max(float(base_delta) * 1.1, 8.0), max_single_probe_delta)
    return min(max(float(base_delta), 8.0), max_single_probe_delta)


def task_probe_perturbation(
    account_store: AccountStore,
    sold_cache: SoldCache,
    rule_engine: RuleEngine,
    imported_store: BatchItemStore,
    ...
    if not bool(getattr(cfg, "probe_enabled", False)):
        _log("probe_perturbation 跳过: probe_enabled=false", on_progress)
        return {"total": 0, "ok": 0, "skip": 0, "fail": 0}

    history_db = get_history_db()
    accounts = {account.name: account for account in account_store.enabled_accounts()}
    all_items = imported_store.get_all()
    items = [
        item for item in all_items
        if str(getattr(item, "manual_review_state", "") or "").strip().lower()
        not in {MANUAL_REVIEW_STATE_REJECTED, MANUAL_REVIEW_STATE_REJECTED_IGNORED}
    ]
    total = ok = skip = fail = 0
    persisted_count = 0
    persisted_samples: list[dict[str, Any]] = []
    ...
    now = datetime.datetime.now()
    _log(
        f"probe_perturbation 商品池: 导入总数 {len(all_items)}，探针候选 {len(items)}（排除永久拒绝/不处理区）",
        on_progress,
    )

    for item in items:
        total += 1
        current_op_status = str(getattr(item, "op_status", "") or "").strip()
        if current_op_status in {"已改价", "待确认"}:
            next_op_status = "待确认" if current_op_status == "待确认" else "跳过"
            imported_store.update_item(
                item.product_id,
                reprice_ok=None,
                reprice_msg="主任务优先，本轮探针让路",
                op_status=next_op_status,
                op_message="探针让路主任务",
            )
            skip += 1
            continue

        if getattr(item, "status", None) != ProductStatus.ON_SALE:
            skip += 1
            continue

        params = _probe_params_for_item(item)
        base_delta = float(params["delta"])
        probe_delta = _effective_probe_delta(item, base_delta)
        interval_hours = int(params["interval_hours"])
        daily_max = int(params["daily_max"])
        due, due_reason = _probe_is_due(item, now, min_hours=interval_hours, daily_max=daily_max)
        if not due:
            imported_store.update_item(item.product_id, reprice_ok=None, reprice_msg=due_reason, op_status="跳过", op_message=f"探针跳过：{due_reason}")
            skip += 1
            continue
        guard_window_hours = _probe_guard_window_hours(item)
        _log(
            f"[{item.account_name}] [{item.qc_code or item.product_id}] 探针分层={params['tier']} delta={probe_delta:.0f}(base={base_delta:.0f}) interval={interval_hours}h daily_max={daily_max} guard_window={guard_window_hours}h",
            on_progress,
        )

        guard_hit, guard_reason = _probe_guard_triggered(item, now=now)
        if guard_hit:
            imported_store.update_item(
                item.product_id,
                reprice_ok=None,
                op_status="待确认",
                op_message=f"探针熔断：{guard_reason}",
                reprice_msg=f"探针熔断：{guard_reason}",
                **_manual_review_fields(
                    {"needs_manual_review": True, "manual_review_reason": f"探针熔断：{guard_reason}"},
                    source="probe_perturbation",
                    target_action="change_price",
                    target_price=getattr(item, "current_price", None),
                ),
            )
            skip += 1
            continue
        ...
        if _is_price_operation_blocked(account.name):
            imported_store.update_item(
                item.product_id,
                reprice_ok=None,
                reprice_msg="账号策略限制：ldy 仅观察，不执行探针改价",
                op_status="跳过",
                op_message="探针跳过：账号策略限制",
            )
            skip += 1
            continue
        svc = services.get(account.name)
        ...
        login_ok, login_reason = _account_login_state(svc, account.name, login_state_cache)
        if not login_ok:
            imported_store.update_item(item.product_id, reprice_ok=False, reprice_msg=f"账号登录失效：{login_reason}", op_status="失败", op_message="探针执行失败")
            fail += 1
            continue

        detail = _refresh_imported_detail(svc, item)
        if detail is None:
            imported_store.update_item(item.product_id, reprice_ok=False, reprice_msg="未找到商品", op_status="失败", op_message="探针执行失败")
            fail += 1
            continue
        ...
        system_target = pipeline.get("suggested_price")
        if system_target is not None and abs(float(system_target) - current_price) >= 1:
            shift = float(system_target) - current_price
            imported_store.update_item(
                item.product_id,
                reprice_ok=None,
                op_status="跳过",
                op_message="主程序命中价已偏移，探针跳过",
                reprice_msg=f"主程序建议价 {float(system_target):.0f}，当前价 {current_price:.0f}，偏移 {shift:+.0f}",
            )
            _log(
                f"[{account.name}] [{detail.qc_code or item.qc_code}] 探针跳过: 主程序命中价偏移 current={current_price:.0f} target={float(system_target):.0f} diff={shift:+.0f}",
                on_progress,
            )
            skip += 1
            continue

        anchor = getattr(item, "probe_anchor_price", None)
        if anchor is None:
            anchor = current_price
        last_side = str(getattr(item, "probe_last_side", "") or "")
        if abs(current_price - float(anchor)) >= probe_delta:
            side = "down" if current_price > float(anchor) else "up"
        else:
            side = "down" if last_side == "up" else "up"
        target = float(anchor) + (probe_delta if side == "up" else -probe_delta)
        target = _normalize_tail8_price(target)
        ...
        decision = recalc_decision_with_final_price(
            ...
            target,
            extra_steps=[f"探针扰动：锚点{float(anchor):.0f}，方向{side}，幅度{probe_delta:.0f}"],
        )
        ...
        if decision.get("needs_manual_review"):
            _mark_manual_review_pending(
                ...
                target_action="change_price",
                target_price=target,
                reprice_msg=f"探针待确认：{decision.get('manual_review_reason') or '风控拦截'}",
                op_message=f"探针待确认：{decision.get('manual_review_reason') or '风控拦截'}",
            )
            skip += 1
            continue

        if target is None or abs(float(target) - current_price) < 1:
            imported_store.update_item(item.product_id, reprice_ok=None, reprice_msg="探针价差小于1元", op_status="跳过", op_message="探针跳过")
            skip += 1
            continue

        success, msg = svc.change_price(detail, target)
        if not success:
            imported_store.update_item(item.product_id, reprice_ok=False, reprice_msg=msg or "探针改价失败", op_status="失败", op_message="探针执行失败")
            fail += 1
            continue
        ...
        persisted_count += 1
        day_key = now.strftime("%Y-%m-%d")
        prev_day = str(getattr(item, "probe_day", "") or "")
        prev_count = int(getattr(item, "probe_day_count", 0) or 0)
        new_count = prev_count + 1 if prev_day == day_key else 1
        imported_store.update_item(
            item.product_id,
            probe_anchor_price=float(anchor),
            probe_last_side=side,
            probe_last_at=now,
            probe_day=day_key,
            probe_day_count=new_count,
            current_price=target,
            suggested_price=target,
            new_price=target,
            reprice_ok=True,
            op_status="已改价",
            op_message=f"探针改价成功（{side}）",
            reprice_msg=msg or "探针改价成功",
        )
        history_db.record(PriceChangeRecord(
            id=None,
            timestamp=now,
            ...
            diff=float(target) - current_price,
            settle_price=PricingEngine.calc_settle_price(float(target)) or 0.0,
            trigger=PriceTrigger.PROBE,
            account_name=account.name,
            note=f"probe:{side}",
        ))
        _append_persisted_sample(
            persisted_samples,
            task="probe_perturbation",
            account=account.name,
            item=detail.qc_code or item.qc_code or item.product_id,
            old_price=current_price,
            new_price=float(target),
            trigger="PROBE",
            model=str(getattr(detail, "model", "") or ""),
            condition=str(getattr(detail, "condition", "") or ""),
            qc_code=str(getattr(detail, "qc_code", "") or item.qc_code or ""),
            imei=str(getattr(detail, "imei", "") or ""),
            settle_price=PricingEngine.calc_settle_price(float(target)),
            changed_at=now,
        )

    _log(f"probe_perturbation 结果: total={total}, ok={ok}, skip={skip}, fail={fail}, 已写入={persisted_count}", on_progress)
    return {
        "total": total,
        "ok": ok,
        "skip": skip,
    ...
    for item in items:
        ...
        if skip_reason:
            imported_store.update_item(
                ...
                op_status=_pending_aware_op_status(skip_reason),
        ...
        if _is_price_operation_blocked(account.name):
            imported_store.update_item(
                item.product_id,
                op_status="跳过",
                op_message="自动上架跳过：账号策略限制",
                reprice_ok=None,
                reprice_msg="账号策略限制：ldy 仅观察，不执行自动上架",
            )
            log(f"[{account.name}] [{item.qc_code or item.product_id}] 跳过: 账号策略限制（仅观察，不执行自动上架）")
            skip += 1
            continue
        ...
        if refreshed_skip_reason:
            imported_store.update_item(
                ...
                op_status=_pending_aware_op_status(refreshed_skip_reason),
        ...
        if _is_small_price_diff(price, detail.current_price):
            _mark_small_diff_skip(
                imported_store,
                product_id=item.product_id,
                account_name=account.name,
...
def task_sales_report(
    ...
    *,
    ...
    wx_app_client=None,
    limit: int = 80,
    on_status_records: Optional[Callable[[list[dict]], None]] = None,
    on_single_status_change: Optional[Callable[[dict], None]] = None,
) -> str:
    """
    生成并发送销售播报。
    ...
    refresh_result = task_refresh_imported_items_status(
        ...
        on_progress=on_progress,
        limit=limit,
        on_status_change_record=on_single_status_change,
    )
    status_records = list(refresh_result.get("status_change_records") or [])
    if on_status_records is not None and status_records:
        try:
            on_status_records(status_records)
        except Exception:
            pass
