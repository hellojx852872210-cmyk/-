...
class ZhuanzhuanImeiQtTab(QWidget):
    ...
    def _build_ui(self):
        ...
        intro = QLabel("该页面用于按 IMEI / 质检码查询单品、查看缓存定价建议、预估到手价，并执行改价 / 上架 / 下架。")
    ...
    def _build_action_box(self):
        ...
        self._withdraw_btn = QPushButton("执行下架")
        ...
        btn_row.addWidget(self._withdraw_btn)
        ...
        self._withdraw_btn.clicked.connect(self._withdraw_product)
    ...
    def _withdraw_product(self):
        self._start_action("withdraw")
    ...
    def _start_action(self, action: str):
        ...
        if not self._current_detail:
            ...
            return
        price = self._parse_new_price()
        if action in {"change", "list"} and price is None:
        ...
        action_text = {"change": "改价", "list": "上架", "withdraw": "下架"}.get(action, action)
        price_text = f" -> {price:.0f}" if price is not None and action in {"change", "list"} else ""
        self._action_status.setText(f"正在执行{action_text}…")
        self._log_action(f"开始{action_text}：{self._current_detail.title}{price_text}")
        threading.Thread(target=self._do_action, args=(action, account_name, price or 0.0), daemon=True).start()
    ...
    def _do_action(self, action: str, account_name: str, price: float):
        ...
        try:
            ...
            if action == "change":
                ...
                ok, message = service.withdraw_product(detail.product_id)
            refreshed_detail = detail
            if ok and action in {"change", "list"}:
    ...
    def _poll_action_result(self):
        ...
        action_text = {"change": "改价", "list": "上架", "withdraw": "下架"}.get(action, str(action or "操作"))
